...
public class FieldDataController extends Application implements GsehenEventListener<FarmDataChanged> {
	...
	public void handle(FarmDataChanged event) {
		...
		if (!soilList.isEmpty()) {
			...
			currentSoilBox.setConverter(new StringConverter<SoilProfile>() {
				...
				public SoilProfile fromString(String string) {
					return currentSoilBox.getItems().stream().filter(ap -> ap.getName().equals(string)).findFirst()
							.orElse(null);
				}
			});
		}
			...
			public void handle(ActionEvent e) {
				if (currentSoilBox.getValue() != null) {
					...
					for (int i = 0; i < currentSoilBox.getValue().getSoilType().size(); i++) {
						...
						center.getChildren().addAll(layor, soil, soilChoiceBox, soilAwcLabel, soilAwc, depthLabel,
								depth);
					}
		...
		createSoil.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				soilChoiceBox.setConverter(new StringConverter<Soil>() {
					...
					@Override
					public Soil fromString(String string) {
						return soilChoiceBox.getItems().stream().filter(ap -> ap.getName().equals(string)).findFirst()
								.orElse(null);
					}
				});
				...
				setSoil.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(ActionEvent arg0) {
						...
						if (soilChoiceBox.getValue() != null && depth.getText() != null
							...
							Text createdSoil = new Text(mainBundle.getString("fieldview.layor") + (layorList.size() + 1)
				...
				center.getChildren().addAll(layorText, soil, soilChoiceBox, soilAwcLabel, soilAwc, depthLabel, depth,
						setSoil);
				ScrollPane scrollPane = new ScrollPane();
				scrollPane.setContent(center);
				scrollPane.setPannable(true);
		...
		if (!wdsList.isEmpty()) {
			...
			weatherData.setConverter(new StringConverter<WeatherDataSource>() {
				...
				public WeatherDataSource fromString(String string) {
					return weatherData.getItems().stream().filter(ap -> ap.getName().equals(string)).findFirst()
							.orElse(null);
				}
			});
		}
		...
		grid.getChildren().addAll(nameLabel, name, areaLabel, area, soilProfile, currentSoilBox, editProfile,
				weatherDataSource, weatherData, editWds, createSoil, createWds, saveField);
		...
		treeTableView = (TreeTableView<Drawable>) Gsehen.getInstance().getScene().lookup(FARM_TREE_VIEW_ID);
	...
	public void getCurrentProfile() {
		...
		int index = 1;
		Text setSoil = new Text(mainBundle.getString("fieldview.currentsoil") + " (" + sp.getName() + "):" + "\n");
	...
	public void setWeatherDataSource() {
		...
		Hyperlink dateFormatExample = new Hyperlink(mainBundle.getString("fieldview.dateformatexample"));
		...
		dateFormatExample.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				getHostServices()
						.showDocument("https://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html");
			}
		});
		...
		center.getChildren().addAll(intervalLabel, interval, windspeedLabel, windspeed, dateFormatLabel, dateFormat,
				dateFormatExample, localeIdLabel, localeId, filePathLabel, path, fileChooserButton, locationLatLabel,
				locationLat, locationLatExample, locationLngLabel, locationLng, locationLngExample, metersAboveLabel,
				metersAbove);
			...
			public void handle(ActionEvent arg0) {
				if (!weatherDataName.getText().trim().isEmpty() && !interval.getText().trim().isEmpty()
					...
					try {
						...
						dateError.setText(mainBundle.getString("fieldview.dateerror") + "\"" + iae.getMessage() + "\"");
	...
	public void setWeatherDataTexts() {
		final WeatherDataSource selectedWeatherDataSource = weatherData.getSelectionModel().getSelectedItem();
		if (selectedWeatherDataSource != null) {
			...
			metersAbove.setText(String.valueOf(selectedWeatherDataSource.getLocationMetersAboveSeaLevel()));
